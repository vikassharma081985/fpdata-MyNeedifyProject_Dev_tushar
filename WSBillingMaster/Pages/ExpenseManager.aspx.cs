...
namespace WSBillingMaster.Pages
{
    public partial class ExpenseManager : System.Web.UI.Page
    {
        ...
        public static string Save(List<Expense> data)
        {
            using (BusinessLogicLayer obj = new BusinessLogicLayer())
            {
                try
                {
					...
					obj.FetchUserId = Convert.ToInt32(HttpUtility.ParseQueryString(query).Get("UserId"));
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static string SearchExpense(string FromDate, string ToDate, int ExpenseId)
        {
            using (BusinessLogicLayer obj = new BusinessLogicLayer())
            {
                try
                {
					...
					int idx = url.IndexOf('?');
					string query = idx >= 0 ? url.Substring(idx) : "";
					string uID = HttpUtility.ParseQueryString(query).Get("UserId");
                    if (uID != null && uID != "")
                        obj.FetchUserId = Convert.ToInt32(uID);
					//obj.FetchUserId = Convert.ToInt32(HttpUtility.ParseQueryString(query).Get("UserId"));
					obj.OrgId = Convert.ToInt32(HttpUtility.ParseQueryString(query).Get("OrgId"));
					obj.EmpId = Convert.ToInt32(HttpUtility.ParseQueryString(query).Get("EmpId"));
					//int ExpStatus;
					if(obj.FetchUserId > 0)
					{
						obj.ExpenseStatus = 1;
					}
					else if(obj.OrgId > 0)
