...
namespace WSBillingMaster.Pages
{
    public partial class AdminList : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["EmployeeId"] != null || Convert.ToInt32(Session["EmployeeId"]) > 0)
            {
                if (!IsPostBack)
                {
                    ...
                    BindData(1, PageSize,UserId, Role);
                }
            }
            else
            {
        ...
        public void BindData(int PageIndex, int PageSize, int userid, int role)
        {
            using (BusinessLogicLayer ObjIHC = new BusinessLogicLayer())
            {
                ...
                using (DataSet dsMain = ObjIHC.GetAdminList(userid, role))
                {
                    tabIndex++;
        ...
        protected void lnkEdit_Command(object sender, CommandEventArgs e)
        {
            ...
            if (e.CommandName == "edit")
                Response.Redirect("../Pages/AddItem.aspx?ItemId=" + ItemId);
        }
        ...
        protected void rptPager_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "Pager")
            {
                ...
                BindData(pageIndex, PageSize, UserId, Role);
            }
        }
    }
}